...
git config --global --replace-all alias.ls 'log --pretty=format:"%C(yellow)%h %Cred%cr %Cblue(%cn)%C(cyan)%d%Creset %s" --decorate'
git config --global --replace-all alias.ll 'log --pretty=format:"%C(yellow)%h %Cred%cr %Cblue(%cn)%C(cyan)%d%Creset %s" --decorate --stat'
